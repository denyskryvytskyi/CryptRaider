...
void ACryptRaiderCharacter::PlayFootstepSound()
{
    const float Speed = GetVelocity().Size();

    // Threshold for walking speed
    constexpr float SoundRepeatInterval = 0.45f;

    // Only play sound if the player is moving
    if (Speed > 0.0f) {
        // Play footstep sound at player's location
        UGameplayStatics::PlaySoundAtLocation(this, FootstepSound, GetActorLocation());

        bCanPlayFootstep = false; // Disable footstep sound temporarily
...
void ACryptRaiderCharacter::Move(const FInputActionValue& Value)
{
    ...
    if (Controller != nullptr) {
        ...
        if (FootstepSound && bCanPlayFootstep) {
            PlayFootstepSound();
        }
    }
}
